...
page = st.sidebar.radio(
    "Select a page",
    ["Overview", "Data Exploration", "SDOH Impact Analysis", "Patient Risk Stratification", "Cluster Analysis", "Recommendations", "Team Collaboration"]
)
...
# Team Collaboration
    ...
    st.header("Team Collaboration & Process")
    
    st.subheader("Our Team's Collaborative Approach")
    ...
    with col1:
        ...
        Our team adopted an agile and collaborative approach to develop this SDOH analysis dashboard:
        
        1. **Initial Data Exploration**: We began by exploring the Snowflake data dictionary to understand available SDOH variables, their distributions, and potential relationships.
        
        2. **Role Assignment**: Team members were assigned specific roles based on their strengths while maintaining flexibility to support each other.
        
        3. **Iterative Development**: We used an iterative approach with regular check-ins to review progress, address challenges, and refine our analyses.
        
        4. **Integrated Analysis**: Rather than working in silos, we integrated our analyses to create a cohesive narrative about SDOH impacts on health outcomes.
        
        5. **Knowledge Sharing**: Throughout the project, we conducted mini-training sessions to share expertise in Snowflake, data visualization, and healthcare analytics.
        """)
    
    with col2:
        team_process = {
            'Stage': ['Data Exploration', 'Analysis Design', 'Dashboard Development', 'Interpretation', 'Recommendations'],
            'Collaboration Score': [8, 9, 7, 10, 9]
        }
        
        df_process = pd.DataFrame(team_process)
        
        fig = px.bar(df_process, x='Stage', y='Collaboration Score', 
                    color='Collaboration Score',
                    color_continuous_scale=px.colors.sequential.Viridis,
                    title="Team Collaboration by Project Stage",
                    labels={"Collaboration Score": "Collaboration Level (1-10)"})
        st.plotly_chart(fig, use_container_width=True)
    ...
    team_members = [
        {
            "name": "Team Member 1", 
            "strengths": "Data Science, Statistical Analysis", 
            "contributions": "Led the correlation and regression analyses of SDOH variables"
        },
        {
            "name": "Team Member 2", 
            "strengths": "Healthcare Analytics, Clinical Knowledge", 
            "contributions": "Provided clinical context and developed actionable recommendations"
        },
        {
            "name": "Team Member 3", 
            "strengths": "Snowflake Expertise, SQL", 
            "contributions": "Built the data pipeline and optimized Snowflake queries"
        },
        {
            "name": "Team Member 4", 
            "strengths": "Data Visualization, UX Design", 
            "contributions": "Designed interactive visualizations and dashboard layout"
        },
        {
            "name": "Team Member 5", 
            "strengths": "Project Management, Communication", 
            "contributions": "Coordinated team efforts and prepared the final presentation"
        }
    ]
    ...
    st.subheader("Tools & Skills Utilized")
    
    tools_skills = {
        'Tool/Skill': ['Snowflake', 'SQL', 'Python', 'Streamlit', 'Statistical Analysis', 
                      'Machine Learning', 'Data Visualization', 'Healthcare Knowledge'],
        'Usage Level': [90, 85, 80, 75, 70, 65, 95, 80]
    }
    
    df_tools = pd.DataFrame(tools_skills)
    
    fig = px.bar(df_tools, x='Tool/Skill', y='Usage Level',
                color='Usage Level',
                color_continuous_scale=px.colors.sequential.Plasma,
                title="Tools and Skills Utilized in Project",
                labels={"Usage Level": "Utilization %"})
    st.plotly_chart(fig, use_container_width=True)
    
    st.subheader("Application of Snowflake Knowledge")
    
    st.markdown("""
    ### How We Leveraged Snowflake
    
    Our team applied the knowledge gained from the instructional sessions on Snowflake in several ways:
    
    1. **Data Integration**: We used Snowflake to integrate the SDOH data with clinical outcome data, creating a comprehensive view of patient profiles.
    
    2. **Query Optimization**: Applied best practices for query construction to efficiently analyze large datasets without performance issues.
    
    3. **Snowflake Chatbot**: Utilized the Snowflake chatbot feature to explore data and quickly identify relevant variables for our analysis.
    
    4. **Data Sharing**: Leveraged Snowflake's secure data sharing capabilities to collaborate effectively while maintaining data governance.
    
    5. **Time-Travel Feature**: Used Snowflake's time-travel capability to compare data states and validate our analysis results.
    
    #### Example Query Used:
    ```sql
    -- Sample query used to analyze relationship between housing status and ER visits
    SELECT 
        h.housing_status,
        AVG(p.er_visits_yearly) as avg_er_visits,
        AVG(p.health_score) as avg_health_score,
        COUNT(*) as patient_count
    FROM patient_metrics p
    JOIN housing_data h ON p.patient_id = h.patient_id
    GROUP BY h.housing_status
    ...
    challenges = [
        {
            ...
            "solution": "Created a unified data model in Snowflake to connect SDOH variables across different domains"
        },
        {
            "challenge": "Statistical Significance of Correlations", 
            "solution": "Applied rigorous statistical testing and validation to ensure meaningful insights"
        },
        {
            "challenge": "Visualization of Multidimensional SDOH Data", 
            "solution": "Developed customized visualizations to represent complex relationships"
        },
        {
            "challenge": "Translating Findings to Clinical Recommendations", 
            "solution": "Collaborated closely with healthcare experts to ensure practical relevance"
        }
    ]
    ...
    st.dataframe(df_challenges, use_container_width=True)
    ...
    1. **Interdisciplinary Approach**: The combination of data science, healthcare expertise, and technical skills was essential for developing meaningful insights.
    
    2. **Data-Driven Healthcare**: Social determinants data provides crucial context for clinical decision-making that wasn't previously available to providers.
    
    3. **Visualization Power**: Interactive visualizations made complex relationships in SDOH data more accessible and actionable.
    
    4. **Iterative Analysis**: Our approach of continuous refinement led to deeper insights than would have been possible with a linear methodology.
    
    5. **Technical-Clinical Bridge**: Building a common language between technical team members and healthcare experts was critical to project success.
    """)
    
    st.info("""
    This collaborative approach enabled us to develop a comprehensive SDOH analysis that provides actionable insights for healthcare providers. By combining our diverse skills and leveraging Snowflake's powerful capabilities, we were able to uncover meaningful patterns in the data and translate them into practical recommendations for improving patient care.
    """)
